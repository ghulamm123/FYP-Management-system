...
if( isset($_SESSION['user_status']) && ($_SESSION['user_status'] == 'teacher') ) {
	if (isset($_POST['response'])) {
		...
		$response = $conn->prepare('SELECT t1.fullname, t1.rollno, t1.enrollment, t1.cgpa, t2.criterion_1, t2.criterion_2, t2.criterion_3, t2.criterion_4, t2.criterion_5, t2.criterion_6 FROM students t1 INNER JOIN midyear t2 ON t1.id = t2.student_id WHERE t1.group_id = :group_id AND t2.external_id = :id');
		$response->bindParam(':group_id', $group_id);
		$response->bindParam(':id', $_SESSION['user_id']);
		$response->execute();

	}

	elseif (isset($_POST['your_group'])) {
		$group_id = $_POST['group_id'];

		$response = $conn->prepare('SELECT t1.fullname, t1.rollno, t1.enrollment, t1.cgpa, t2.criterion_1, t2.criterion_2, t2.criterion_3, t2.criterion_4, t2.criterion_5, t2.criterion_6 FROM students t1 INNER JOIN midyear t2 ON t1.id = t2.student_id WHERE t1.group_id = :group_id AND t2.internal_id = :id');
		$response->bindParam(':group_id', $group_id);
		$response->bindParam(':id', $_SESSION['user_id']);
		$response->execute();
...
<body>
		...
		<br /><br />
		<p style='text-align: center'><font size="5">Mid-Year Response:</font></p>
