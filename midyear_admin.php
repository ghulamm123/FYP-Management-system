...
if( isset($_SESSION['user_status']) && ($_SESSION['user_status'] == 'admin') ) {
	$assign_external = $conn->prepare('SELECT t1.username, t2.group_id, t2.internal_id, t2.filename, t2.description, t2.date_time, FROM groups t1 INNER JOIN proposals t2 ON t1.id = t2.group_id WHERE t1.midyear_external_id IS NULL');
	$assign_external->execute();

	$groups_external = $conn->prepare('SELECT t1.username, t2.group_id, t2.filename, t2.description, t2.date_time, t3.fullname, t3.designation FROM groups t1 INNER JOIN proposals t2 ON t1.id = t2.group_id INNER JOIN internals t3 ON t1.midyear_external_id = t3.id WHERE t1.midyear_external_id IS NOT NULL');
...
<body>
		...
		<br /><br />
		<p><font size="5">Assign External To Projects/Groups For Midyear:</font></p>
		...
		<button type="submit" name="assign_midyear_external">Assign</button>
		</p>
		</form> </div> <br>
		<?php
		}

		echo "<p><font size='5'>Externals & Response Of The Projects/Groups For Midyear:</font></p>";
		...
		<p style='text-align: right'>
		<input type="hidden" name="group_id" value="<?php echo $results['group_id']; ?>">
		<button type="submit" name="midyear_response">Response</button>
		</p>
		</form> </div> <br>
		<?php
		}
